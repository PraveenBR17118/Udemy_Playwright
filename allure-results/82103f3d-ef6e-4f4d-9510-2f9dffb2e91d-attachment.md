...
- Name: ClientAppOtherWay.spec.js >> @Webst Client App login
...
```ts
  ...
  2  | //const {expect} = require('../playwright.config');
  3  | 
  4  | 
  5  | test('@Webst Client App login', async ({ page }) => {
  6  |     //js file- Login js, DashboardPage
  7  |     const email = "tanvitkashyap@gmail.com";
...
> 10 |     await page.goto("https://rahulshettyacademy.com/client");
  ...
  12 |     await page.getByPlaceholder("enter your passsword").fill("Pp@12345");
  13 |     await page.getByRole('button',{name:"Login"}).click();
  14 |     await page.waitForLoadState('networkidle');
  15 |     await page.locator(".card-body b").first().waitFor();
  16 |     
  17 |     await page.locator(".card-body").filter({hasText:"ZARA COAT 3"})
  18 |     .getByRole("button",{name:"Add to Cart"}).click();
  19 |   
  20 |     await page.getByRole("listitem").getByRole('button',{name:"Cart"}).click();
  21 |   
  22 |     //await page.pause();
  23 |     await page.locator("div li").first().waitFor();
  24 |     await expect(page.getByText("ZARA COAT 3")).toBeVisible();
  25 |   
  26 |     await page.getByRole("button",{name :"Checkout"}).click();
  27 |   
  28 |     await page.getByPlaceholder("Select Country").pressSequentially("ind");
  29 |   
  30 |     await page.getByRole("button",{name :"India"}).nth(1).click();
  31 |     await page.getByText("PLACE ORDER").click();
  32 |   
  33 |     await expect(page.getByText("Thankyou for the order.")).toBeVisible();
  34 |  })
```